...
def _parse_cimb_transaction_details_text(raw: str) -> List[ParsedRow]:
    """Parse CIMB Transaction Details payment export report from raw PDF text.

    pdfplumber extracts the table header row but not the data rows for this
    format. Each transaction spans 2-3 raw text lines:
      Line 1: {DD Mon} {DD Mon} MAIA SDN. - {MDNpfx} - {name_part1} - {account} MYR ...
      Line 2: {YYYY} {HH}: {YYYY} BHD. {ref_sfx} {name_part2} (MYR) {amount} ...
      Line 3+: [{HH:MM MYT} {name_continuation...}]
    """
    lines = raw.split("\n")
    # Each transaction starts with "DD Mon DD Mon MAIA SDN."
    txn_start_re = re.compile(r"^(\d{1,2} \w{3})\s+(\d{1,2} \w{3})\s+MAIA")
    # Line 2 pattern: YYYY HH: YYYY BHD. {ref_suffix} {name_mid} (MYR) {amount}
    line2_re = re.compile(
        r"^(\d{4})\s+\d+:\s+\d{4}\s+BHD\.\s+(\d+)\s+(.*?)\s*\(MYR\)\s+([\d,]+\.\d{2})"
    )
    time_cont_re = re.compile(r"^\d{2}:\d{2}\s+MYT\s*")
    ...
    while i < len(lines):
        ...
        payment_date_part1 = m.group(2)  # second "DD Mon" = Payment Date
        line1 = lines[i]

        # Collect this transaction's block until next transaction starts
        j = i + 1
        block = [line1]
        while j < len(lines):
            nxt = lines[j]
            if txn_start_re.match(nxt) or nxt.startswith("Generated By") or nxt.startswith("Generated On"):
                break
            block.append(nxt)
            j += 1

        # Parse line 2
        line2_m = line2_re.match(block[1]) if len(block) > 1 else None
        if not line2_m:
            i = j
            continue

        year, ref_suffix, name_from_line2, amount_str = (
            line2_m.group(1), line2_m.group(2), line2_m.group(3).strip(), line2_m.group(4)
        )

        txn_date = _parse_date(f"{payment_date_part1} {year}")
        if not txn_date:
            i = j
            continue
        ...
        # Reference: MDN prefix (from line 1) + numeric suffix (from line 2)
        ref1_m = re.search(r"(MDN\d+)", line1)
        ref1 = ref1_m.group(1) if ref1_m else ""
        reference = (ref1 + ref_suffix) if ref1 else ref_suffix or None

        # Beneficiary part from line 1: text between last " - " and the debit-account number
        acc_m = re.search(r"-\s+(\d{10})", line1)
        name_from_line1 = ""
        if acc_m:
            before_acc = line1[: acc_m.start()]
            ld = before_acc.rfind(" - ")
            if ld >= 0:
                name_from_line1 = before_acc[ld + 3:].strip()

        # Beneficiary continuation from lines 3+ (strip time "MM:SS MYT" prefix)
        name_from_cont = ""
        for cont in block[2:]:
            stripped = time_cont_re.sub("", cont).strip()
            if stripped:
                name_from_cont += " " + stripped

        name_parts = [p for p in [name_from_line1, name_from_line2, name_from_cont.strip()] if p]
        beneficiary = re.sub(r"\s+", " ", " ".join(name_parts)).strip()

        results.append(ParsedRow(
            txn_date=str(txn_date),
            description=beneficiary or "DUITNOW",
            party_name=beneficiary or None,
        ...
        with pdfplumber.open(io.BytesIO(file_bytes)) as pdf:
            for page in pdf.pages:
                ...
                if not page_had_table:
                    ...
                    # CIMB Transaction Details (payment export): has "Transaction Details" heading,
                    # MDN reference numbers, and "(MYR)" amount markers.
                    if ("Transaction Details" in text_content
                            and re.search(r"MDN\d+", text_content)
                            and re.search(r"\(MYR\)", text_content)):
                        txn_rows = _parse_cimb_transaction_details_text(text_content)
                        if txn_rows:
                            results.extend(txn_rows)
                            continue
